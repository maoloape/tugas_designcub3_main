...
(function (factory) {
    ...
    L.Control.Search = L.Control.extend({
        ...
        options: {
            ...
            propertyAddress: "address", // property in marker.options(or feature.properties for vector layer) trough filter elements in layer
        ...
        _createTip: function (text, val) {
            // val is an object containing title, address, lat, lng
            let tip;

            if (this.options.buildTip) {
                // Use custom buildTip function if provided
                ...
                // Create a default tip if no custom buildTip is provided
                tip = L.DomUtil.create("li", "search-tip");
                tip.innerHTML = `
                <strong>${val.layer.options.title || "No Title"}</strong><br>
                ${val.layer.options.address || "No Address"}<br>
            `;
            }

            L.DomUtil.addClass(tip, "search-tip");
            tip._text = text; // Store the original text for potential use
            ...
            if (this.options.tooltipLimit) {
                for (const key in records) {
                    ...
                    this._tooltip.appendChild(
                        this._createTip(key, records[key])
                    );
                }
            }
        ...
        _defaultFormatData: function (json) {
            ...
            const propAddress = this.options.propertyAddress;
            const propLoc = this.options.propertyLoc;
            const jsonret = [];

            for (const i in json) {
                // Mendapatkan title, address, dan lokasi
                const title = self._getPath(json[i], propName);
                const address = self._getPath(json[i], propAddress);
                let latLng;

                // Menentukan nilai latLng
                if (L.Util.isArray(propLoc)) {
                    ...
                    latLng = L.latLng(self._getPath(json[i], propLoc));
                }

                // Menyimpan title, address, dan latLng dalam satu objek
                jsonret.push({
                    title: title,
                    address: address,
                    latLng: latLng,
    ...
    */
        ...
        _searchInLayer: function (layer, retRecords, propName, propAddress) {
            const self = this;
            let loc;

            if (layer instanceof L.Control.Search.Marker) return;

            // Function to get the key based on propertyName and propertyAddress
            const getKey = function (layer) {
                const name =
                    self._getPath(layer.options, propName) ||
                    self._getPath(layer.feature.properties, propName);
                const address =
                    self._getPath(layer.options, propAddress) ||
                    self._getPath(layer.feature.properties, propAddress);
            ...
            if (layer instanceof L.Marker || layer instanceof L.CircleMarker) {
                ...
                if (key) {
                    ...
                    console.warn(
                        `propertyName '${propName}' or propertyAddress '${propAddress}' not found in marker`
                    );
                }
            } else if (
                ...
                if (key) {
                    ...
                    console.warn(
                        `propertyName '${propName}' or propertyAddress '${propAddress}' not found in shape`
                    );
                }
                ...
                if (
                    ...
                    if (
                        ...
                        loc = layer.getLatLng();
                        loc.layer = layer;
                        const key = `${layer.feature.properties[propName]}: ${layer.feature.properties[propAddress]}`;
                        retRecords[key] = {
                            lat: loc.lat,
                            lng: loc.lng,
                            layer: layer,
                    ...
                    console.warn(
                        `propertyName '${propName}' or propertyAddress '${propAddress}' not found in feature`
                    );
                }
        ...
        _recordsFromLayer: function () {
            ...
            this._layer.eachLayer(function (layer) {
                self._searchInLayer(layer, retRecords, propName, propAddress);
            });
        ...
        _autoType: function () {
            ...
            const firstRecord = this._tooltip.firstChild
        ...
        _fillRecordsCache: function () {
            ...
            if (this.options.layer) {
                ...
                this._curReq = this._retrieveData(
                    this,
                    inputText,
                    function (data) {
        ...
        _handleArrowSelect: function (velocity) {
            const searchTips = this._tooltip.hasChildNodes()
            ...
            if (
                ...
                this._input.value =
                    searchTips[this._tooltip.currentSelection]._text;

                // scroll:
                const tipOffsetTop =
                    searchTips[this._tooltip.currentSelection].offsetTop;
    ...
    L.Control.Search.Marker = L.Marker.extend({
        ...
        initialize: function (latlng, options) {
            ...
            if (L.Control.Search.prototype._isObject(this.options.circle)) {
                this._circleLoc = new L.CircleMarker(
                    latlng,
                    this.options.circle
                );
            }
